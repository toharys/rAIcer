...
class RobotServer:
    ...
    def _init_display_check(self):
        """Check if we're running on local display (not SSH)"""
        # Only enable visualization if DISPLAY is set (local Jetson session)
        self.local_display = os.getenv('DISPLAY') is not None
        if self.local_display:
            print("Local display detected - enabling visualization")
        else:
            print("No local display (likely SSH) - disabling visualization")
    ...
    def _visualization_thread_func(self):
        ...
        try:
            cv2.namedWindow('Jetson Camera View', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('Jetson Camera View', 640, 480)
            ...
            while not self.terminate:
                ...
                cv2.putText(display_frame, 
                           f"Action: {self.current_action.name}",
                           (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 
                           0.8, (0, 255, 0), 2)
                
                cv2.imshow('Jetson Camera View', display_frame)
    ...
    def run_server(self):
        """Start visualization only if local display exists"""
        if self.local_display:
            vis_thread = threading.Thread(
                target=self._visualization_thread_func,
                daemon=True
            )
            vis_thread.start()
